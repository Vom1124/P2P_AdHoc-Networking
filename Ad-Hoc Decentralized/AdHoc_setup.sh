...
sudo wpa_supplicant -B -c /etc/wpa_supplicant/wpa_supplicant.conf -Dwext -i wlan1
...
echo "\e[96m Displaying the available broadcasting SSIDs for wlan0\e[0m"
...
# ---Check the if the network is available and broadcasting its SSID (wlan0)
...
printf "%s" "Press enter to continue to scan wlan0 network SSIDs"
read ans

sleep 2
echo "\e[96m Now, Displaying the available broadcasting SSIDs for wlan1\e[0m"
...
echo "\e[93mCheck if the built-in Wi-Fi module 'wlan0' is connected to the desired internet\e[0m\n"
sleep 1
sudo iwgetid wlan0
sleep 1
...
echo "\e[93mCheck if the wifi USB dongle: 'wlan1' is connected to the AdHoc network\e[0m\n"
sleep 1
sudo iwgetid wlan1
sleep 1
